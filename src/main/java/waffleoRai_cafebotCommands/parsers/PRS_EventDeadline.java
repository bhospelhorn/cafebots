...
public class PRS_EventDeadline implements Parser{
	...
	public Command generateCommand(String[] args, MessageReceivedEvent event) 
	{
		...
		CMD_EventMakeDeadline cmd = null;
		if (group)
		{
			cmd = new CMD_EventMakeDeadline(event.getChannel(), event.getMember(), event.getMessageIdLong());
		}
		else
		{
			cmd = new CMD_EventMakeDeadline(event.getChannel(), event.getMember(), tusers, event.getMessageIdLong());
		}
